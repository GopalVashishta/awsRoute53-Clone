...
export default function HostedZones() {
  ...
  const loadZones = async () => {
    setLoading(true);
    try{ //encodeURIComponent safely attaches the string to URL
  ...
  return (
    <AppShell>
      <div className="aws-content-area">
        ...
        <div className="aws-table-container">
          <div style={{ padding: 16, borderBottom: '1px solid #eaeded' }}>
            <input type="text" placeholder="Search hosted zones" value={search} 
              onChange={e => { setSearch(e.target.value); setPage(1); }} 
              className="aws-input" style={{ maxWidth: 300 }} />
          </div>
          
          {loading ? (
            <div style={{ padding: 40, textAlign: 'center' }}><Spinner /></div>
          ) : zones.length === 0 ? (
            <EmptyState title="No hosted zones found" subtitle="You don't have any hosted zones matching your criteria. Create one to get started." />
          ) : (
            <>
                ...
                <tbody>
                  {zones.map(zone => (
                    <tr key={zone.id} className={selected.includes(zone.id) ? 'selected' : ''}>
                      ...
                      <td><Link href={`/hosted-zones/${zone.id}`} style={{color:'#0972d3',textDecoration:'none',fontWeight:500}}>{zone.name}</Link></td>
                      <td><span className={zone.is_private_zone ? 'aws-badge-private' : 'aws-badge-public'}>{zone.is_private_zone ? 'Private' : 'Public'}</span></td>
                      <td>{zone.record_set_count ?? zone.record_count ?? 0}</td>
                      <td>{zone.comment || '-'}</td>
                      <td>{zone.id}</td>
                    </tr>
                  ))}
      ...
      <Modal isOpen={deleteModal} 
            ...
            title="Delete hosted zones" footer={
            <>
              <Button onClick={() => setDeleteModal(false)}>Cancel</Button>
              <Button variant="danger" loading={deleting} onClick={handleDelete}>Delete</Button>
            </>
            }> 
            {/*pass the footer for this too */}
