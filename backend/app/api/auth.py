...
def login(data: LoginRequest, response: Response, db: Session = Depends(get_db)):
    ...
    response.set_cookie(key="session_id", value=token, httponly=True, samesite="none", secure=True, path="/")
...
def logout(request: Request, response: Response, db: Session = Depends(get_db)):
    ...
    response.delete_cookie(key="session_id", path="/", samesite="none", secure=True, httponly=True)
